...
def test_store_tables():
    ...
    store1, created = Store.objects.get_or_create(
        name="Test Store 1", defaults={"user": user}
    )
    if created:
    ...
    store2, created = Store.objects.get_or_create(
        name="Test Store 2", defaults={"user": user}
    )
    if created:
    ...
    with connection.cursor() as cursor:
        ...
        sql1 = "SELECT name FROM sqlite_master WHERE type='table' AND name LIKE '{}'".format(
            pattern1.replace("%", "%%")
        )
        cursor.execute(sql1)
        ...
        sql2 = "SELECT name FROM sqlite_master WHERE type='table' AND name LIKE '{}'".format(
            pattern2.replace("%", "%%")
        )
        cursor.execute(sql2)
    ...
    try:
        ...
        print(
            f"✓ Created category '{cat1_store1.name}' in Store {store1.id} (ID: {cat1_store1.id})"
        )

        cat1_store2 = StoreCategory.objects(store2.id).create(name="Electronics")
        print(
            f"✓ Created category '{cat1_store2.name}' in Store {store2.id} (ID: {cat1_store2.id})"
        )
    ...
    try:
        ...
        print(
            f"✓ Created image '{img1_store1.name}' in Store {store1.id} (Code: {img1_store1.image_code})"
        )
        ...
        print(
            f"✓ Created image '{img1_store2.name}' in Store {store2.id} (Code: {img1_store2.image_code})"
        )
