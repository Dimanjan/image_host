...
from images.store_helpers import StoreCategory, StoreImage, StoreProduct
...
class StoreTableOrganizationTest(TestCase):
    ...
    def test_store_tables_created(self):
        ...
        with connection.cursor() as cursor:
            ...
            cursor.execute(
                "SELECT name FROM sqlite_master WHERE type='table' AND name LIKE 'store_1_%'"
            )
            ...
            cursor.execute(
                "SELECT name FROM sqlite_master WHERE type='table' AND name LIKE 'store_2_%'"
            )
    ...
    def test_filtering_and_search(self):
        ...
        prod1 = StoreProduct.objects(self.store1.id).create(
            category=cat1, name="iPhone"
        )
        prod2 = StoreProduct.objects(self.store1.id).create(category=cat1, name="iPad")
        prod3 = StoreProduct.objects(self.store1.id).create(
            category=cat2, name="T-Shirt"
        )

        # Test filtering by category
        electronics_products = StoreProduct.objects(self.store1.id).filter(
            category_id=cat1.id
        )
        self.assertEqual(len(electronics_products), 2)

        # Test name search (icontains)
        phone_products = StoreProduct.objects(self.store1.id).filter(
    ...
    def test_multiple_stores_same_data(self):
        ...
        self.assertNotEqual(
            store1_prod.id, store2_prod.id
        )  # Different IDs in different tables
...
class StoreTableIntegrationTest(TestCase):
    ...
    def test_store_creation_creates_tables(self):
        ...
        response = self.client.post("/stores/create/", {"name": "New Test Store"})
    ...
    def test_category_creation_uses_store_tables(self):
        ...
        response = self.client.post(
            f"/categories/create/{store.id}/", {"name": "Test Category"}
        )
    ...
    def test_product_creation_uses_store_tables(self):
        ...
        response = self.client.post(
            f"/stores/{store.id}/categories/{category.id}/products/create/",
            {"name": "iPhone 15"},
        )
