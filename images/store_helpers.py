...
class StoreCategory:
    ...
    def save(self):
        ...
        with connection.cursor() as cursor:
            if self.id:
                # Update
                sql = "UPDATE {} SET name = ?, updated_at = ? WHERE id = ?".format(self.table_name)

                cursor.execute(sql, [self.name, now, self.id])
            else:
                # Create
                sql = "INSERT INTO {} (name, created_at, updated_at) VALUES (?, ?, ?)".format(self.table_name)
...
class StoreCategoryManager:
    ...
    def all(self):
        with connection.cursor() as cursor:
            ...
            return [StoreCategory(self.store_id, dict(zip(columns, row))) for row in cursor.fetchall()]
    ...
    def filter(self, **kwargs):
        with connection.cursor() as cursor:
            ...
            sql = "SELECT * FROM {} WHERE {} ORDER BY name".format(self.table_name, where_clause)
            cursor.execute(sql, params)
            columns = [col[0] for col in cursor.description]
            return [StoreCategory(self.store_id, dict(zip(columns, row))) for row in cursor.fetchall()]
...
class StoreProduct:
    ...
    def category(self):
        if not self._category and self.category_id:
            try:
                self._category = StoreCategory.objects(self.store_id).get(id=self.category_id)
            except:
                self._category = None
...
class StoreProductManager:
    ...
    def all(self):
        with connection.cursor() as cursor:
            ...
            cursor.execute(sql)
            columns = [col[0] for col in cursor.description]
            return [StoreProduct(self.store_id, dict(zip(columns, row))) for row in cursor.fetchall()]
...
class StoreImage:
    ...
    def product(self):
        if not self._product and self.product_id:
            try:
                self._product = StoreProduct.objects(self.store_id).get(id=self.product_id)
            except:
                self._product = None
    ...
    def save(self):
        from .models import generate_image_code
        import re
        ...
        counter = 1
        while True:
            existing_images = StoreImage.objects(self.store_id).filter(image_code=self.image_code)
            if not existing_images:
                break
            # Check if it's the same record (update case)
        ...
        if isinstance(self.image_file, UploadedFile):
            ...
            file_path = default_storage.save(f'images/store_{self.store_id}/{self.image_file.name}', self.image_file)
        ...
        with connection.cursor() as cursor:
            if self.id:
                # Update
                if file_path:
                    sql = "UPDATE {} SET product_id = ?, name = ?, image_code = ?, image_file = ?, updated_at = ? WHERE id = ?".format(self.table_name)
                    cursor.execute(sql, [self.product_id, self.name, self.image_code, file_path, now, self.id])
                else:
                    sql = "UPDATE {} SET product_id = ?, name = ?, image_code = ?, updated_at = ? WHERE id = ?".format(self.table_name)
                    cursor.execute(sql, [self.product_id, self.name, self.image_code, now, self.id])
            else:
                # Create
                if not file_path:
                    file_path = self.image_file if isinstance(self.image_file, str) else ''
                sql = "INSERT INTO {} (product_id, name, image_code, image_file, created_at, updated_at) VALUES (?, ?, ?, ?, ?, ?)".format(self.table_name)
                cursor.execute(sql, [self.product_id, self.name, self.image_code, file_path, now, now])
                self.id = cursor.lastrowid
                self.created_at = now
            self.updated_at = now
...
class StoreImageManager:
    ...
    def all(self):
        with connection.cursor() as cursor:
            ...
            cursor.execute(sql)
            columns = [col[0] for col in cursor.description]
            return [StoreImage(self.store_id, dict(zip(columns, row))) for row in cursor.fetchall()]
    ...
    def filter(self, **kwargs):
        with connection.cursor() as cursor:
            ...
            sql = "SELECT * FROM {} WHERE {} ORDER BY created_at DESC".format(self.table_name, where_clause)
            cursor.execute(sql, params)
            columns = [col[0] for col in cursor.description]
            return [StoreImage(self.store_id, dict(zip(columns, row))) for row in cursor.fetchall()]
    ...
    def exists(self, **kwargs):
        with connection.cursor() as cursor:
            ...
            for key, value in kwargs.items():
                ...
                params.append(value)
            where_clause = " AND ".join(conditions)
            sql = "SELECT COUNT(*) FROM {} WHERE {}".format(self.table_name, where_clause)
            cursor.execute(sql, params)
